...
-- Wipe seed transactional data (FK-safe). Audit trigger disabled around delete.
DO $$
DECLARE
...
BEGIN
  ...
  CREATE TEMP TABLE _seed_contracts ON COMMIT DROP AS
  SELECT unnest(seed_contract_ids) AS id;

  UPDATE public.engagement_inquiries
  SET contract_id = NULL, deposit_id = NULL
  WHERE id = ANY (seed_inquiry_ids)
     OR contract_id IN (SELECT id FROM _seed_contracts)
     OR deposit_id IN (SELECT id FROM public.deposits WHERE contract_id IN (SELECT id FROM _seed_contracts));

  DELETE FROM public.customer_vendor_quote_offers WHERE inquiry_id = ANY (seed_inquiry_ids);
  DELETE FROM public.vendor_quotes WHERE rfq_id IN (SELECT id FROM public.vendor_rfqs WHERE inquiry_id = ANY (seed_inquiry_ids));
  ...
  WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.po_approvals WHERE contract_id IN (SELECT id FROM _seed_contracts);

  DELETE FROM public.payment_applications
  WHERE invoice_id IN (SELECT id FROM public.invoices WHERE contract_id IN (SELECT id FROM _seed_contracts))
     OR payment_id::text LIKE '55555555-%';
  DELETE FROM public.payment_drafts WHERE invoice_id IN (SELECT id FROM public.invoices WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.billing_alerts WHERE invoice_id IN (SELECT id FROM public.invoices WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.ar_ledger_entries WHERE invoice_id IN (SELECT id FROM public.invoices WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.ar_bucket_state WHERE invoice_id IN (SELECT id FROM public.invoices WHERE contract_id IN (SELECT id FROM _seed_contracts));

  UPDATE public.billable_time_entries SET billed_invoice_id = NULL WHERE contract_id IN (SELECT id FROM _seed_contracts);
  UPDATE public.billable_costs SET billed_invoice_id = NULL WHERE contract_id IN (SELECT id FROM _seed_contracts);
  UPDATE public.contract_milestones SET billed_invoice_id = NULL WHERE contract_id IN (SELECT id FROM _seed_contracts);

  DELETE FROM public.recognition_evidence WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.invoice_lines WHERE invoice_id IN (SELECT id FROM public.invoices WHERE contract_id IN (SELECT id FROM _seed_contracts));
  UPDATE public.deposits SET applied_to_invoice_id = NULL WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.deposits WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.invoices WHERE contract_id IN (SELECT id FROM _seed_contracts);
  ...
  WHERE cost_entry_id IN (SELECT id FROM public.cost_entries WHERE contract_id IN (SELECT id FROM _seed_contracts))
     OR kept_cost_entry_id IN (SELECT id FROM public.cost_entries WHERE contract_id IN (SELECT id FROM _seed_contracts))
     OR voided_cost_entry_id IN (SELECT id FROM public.cost_entries WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.cost_entry_history WHERE cost_entry_id IN (SELECT id FROM public.cost_entries WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.cost_entries WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.cost_entries WHERE id::text LIKE 'cccccccc-%' OR entered_by IN ('seed-hist', 'seed-demo');
  DELETE FROM public.cost_budgets WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.cost_classifications WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.billable_costs WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.billable_time_entries WHERE contract_id IN (SELECT id FROM _seed_contracts);

  DELETE FROM public.work_obligation_resources
  WHERE contract_id IN (SELECT id FROM _seed_contracts)
     OR obligation_id IN (SELECT id FROM public.work_performance_obligations WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.work_attachments
  WHERE assignment_id IN (SELECT id FROM public.work_assignments WHERE contract_id IN (SELECT id FROM _seed_contracts))
     OR exception_id IN (SELECT id FROM public.work_exceptions WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.work_time_materials WHERE assignment_id IN (SELECT id FROM public.work_assignments WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.work_completions WHERE assignment_id IN (SELECT id FROM public.work_assignments WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.work_assignments WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.work_exceptions WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.work_performance_obligations WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.work_contract_documents WHERE contract_id IN (SELECT id FROM _seed_contracts);

  DELETE FROM public.customer_approval_decisions
  WHERE approval_item_id IN (SELECT id FROM public.customer_approval_items WHERE contract_id IN (SELECT id FROM _seed_contracts));
  DELETE FROM public.customer_approval_items WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_involvement_checkpoints WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_performance_obligations WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_modification_line_items
  WHERE modification_id IN (SELECT id FROM public.contract_modifications WHERE contract_id IN (SELECT id FROM _seed_contracts));
  UPDATE public.contract_documents SET modification_id = NULL WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_modifications WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_documents WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_deliverables WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_line_items WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_milestones WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.contract_approvals WHERE contract_id IN (SELECT id FROM _seed_contracts);
  DELETE FROM public.billing_schedules WHERE contract_id IN (SELECT id FROM _seed_contracts);

  UPDATE public.contracts SET project_manager_party_id = NULL WHERE id IN (SELECT id FROM _seed_contracts);
END $$;

-- Clear append-only audit rows for seed contracts (trigger off only for this statement)
ALTER TABLE public.contract_audit_events DISABLE TRIGGER trg_contract_audit_events_no_update;
DELETE FROM public.contract_audit_events
WHERE contract_id::text LIKE '22222222-%'
   OR contract_id::text LIKE 'e3333333-%'
   OR contract_id = '152268fa-b897-444a-b032-8e65dc4d0b93'::uuid;
ALTER TABLE public.contract_audit_events ENABLE TRIGGER trg_contract_audit_events_no_update;
...
-- Absorb shared overhead into budgets (keeps v_profit_exceptions from flooding)
UPDATE public.cost_budgets cb
SET budgeted_amount = round(cb.budgeted_amount + coalesce(oh.overhead, 0) *
  CASE cb.category
    WHEN 'labor' THEN 0.35
    WHEN 'vendor' THEN 0.45
    WHEN 'equipment' THEN 0.20
    ELSE 0
  END, 2)
FROM (
  SELECT contract_id, sum(amount) AS overhead
  FROM public.cost_entries
  WHERE category = 'allocated' AND commitment_status = 'actual' AND entered_by = 'seed-hist'
  GROUP BY contract_id
) oh
WHERE cb.contract_id = oh.contract_id;

UPDATE public.cost_budgets cb
SET budgeted_amount = round(cb.budgeted_amount * 1.06, 2)
WHERE cb.contract_id::text LIKE '22222222-2222-2222-2222-000000005%'
  AND cb.contract_id NOT IN (
    SELECT DISTINCT contract_id FROM public.cost_entries
    WHERE flag_over_committed = true OR entered_by = 'seed-demo'
  );
